...
URL_FOR_GEO_IP='https://githubcom.pages.dev/Loyalsoldier/v2ray-rules-dat/releases/latest/download/geoip.dat'
URL_FOR_GEO_SITE='https://githubcom.pages.dev/Loyalsoldier/v2ray-rules-dat/releases/latest/download/geosite.dat'
...
install() {
    bash <(curl -Ls https://rawgithub-2kz.pages.dev/Huibq/Crontab/main/freev2/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
install_bbr() {
    ...
    bash <(curl -L -s https://rawgithub-2kz.pages.dev/teddysun/across/master/bbr.sh)
    echo ""
    before_show_menu
}

update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://rawgithub-2kz.pages.dev/Huibq/Crontab/main/freev2/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
